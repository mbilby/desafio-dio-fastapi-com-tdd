...
[DIO](https://github.com/digitalinnovationone/store_api)
...
- Update
    ...
    - ao alterar um dado, a data de updated_at deve corresponder ao time atual, permitir modificar updated_at também
- Tests
    - A suíte de testes foi reformulada com o uso de Pytest como framework principal, aproveitando a abordagem TDD (Test-Driven Development).
- Deploy
    - Dockerfile para construção da imagem da aplicação.
    - docker-compose.yml para orquestração da aplicação com o serviço do MongoDB.
    - entrypoint.sh para inicialização do container com suporte à definição de variáveis de ambiente.
    - A aplicação agora pode ser executada e testada de forma consistente em qualquer ambiente através      de containers, simplificando o processo de CI/CD e evitando problemas de dependência local.
